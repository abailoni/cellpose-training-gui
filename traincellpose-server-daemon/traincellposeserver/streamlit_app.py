...
def start_cellpose_training(training_folder):
    ...
    if not os.path.exists(training_folder) or not os.path.exists(training_images_dir) or not os.path.exists(
        ...
        return False, "Training data or config not found!"
...
if __name__ == '__main__':
    ...
    if uploaded_file is not None:
        ...
        # Start training:
        training_was_successful, error_message = start_cellpose_training(training_dir)
        ...
        with open(out_zip_file, "rb") as fp:
            btn = st.download_button(
                label="Download ZIP trained model",
                data=fp,
